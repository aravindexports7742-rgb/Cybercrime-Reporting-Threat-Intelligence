...
st.subheader(":material/list_alt: All complaints")
...
if response.status_code == 200:
    ...
    if not complaints:
        ...
        # Pills multi-filter for statuses
        unique_statuses = sorted(set(statuses))
        selected_statuses = st.pills(
            "Filter by status", unique_statuses, selection_mode="multi", key="officer_status_pills"
        )
